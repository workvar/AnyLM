...
Sign-in (email/password, Google, GitHub) is handled by Firebase Auth. Orgs, policies and usage limits are served by `src/main/api/`, which runs in this process and reads Firestore directly under the signed-in user's ID token; `../firebase/firestore.rules` is what authorizes it. Nothing has to be started locally first. Project coordinates live in `app/.env` (copy `.env.example`); point the app at a different project or the emulator by changing `ANYLM_FIREBASE_PROJECT`, `ANYLM_FIREBASE_API_KEY`, and `ANYLM_SITE_URL` there.
...
| Secret | Home |
| --- | --- |
| Google / GitHub sign-in client secrets | inside the Firebase project, used by its hosted OAuth handler |
| Outlook connector | public client id in `app/.env`; PKCE, no secret exists |
