...
function parseChromaUrl(url: string): { host: string; port: number; ssl: boolean } | null {
  try {
    const u = new URL(url);
...
// on the local server.
function clientFor(logicalName: string): any {
...
async function getCollection(logicalName: string): Promise<any> {
...
async function remove(name: string, where?: Record<string, unknown>): Promise<boolean> {
...
async function clearCollection(logicalName: string): Promise<boolean> {
  const name = resolveName(logicalName);
...
async function count(name: string, where?: Record<string, unknown>): Promise<number> {
  const col = await getCollection(name);
  if (!col) return 0;
  try {
...
export { GENERAL, PROJECT_CONTEXT, PROJECT_MEMORY, ORG_SHARED, addTexts, queryText, remove, clearCollection, count, available, newId };
