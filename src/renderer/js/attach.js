...
async function openCamera() {
  ...
  try {
    ...
    el("camera-error").textContent = `Camera unavailable: ${e.message}`;
  }
}
...
export function initAttach() {
  ...
  el("attach-doc-input").onchange = async (e) => {
    for (const f of e.target.files) await addDocFile(f);
    e.target.value = "";
    renderChips();
  };
  el("attach-folder-input").onchange = async (e) => {
    for (const f of e.target.files) if (TEXT_EXT.test(f.name)) await addDocFile(f);
    e.target.value = "";
    renderChips();
  };
