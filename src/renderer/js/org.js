...
async function refresh() {
  ...
  [s.limits, s.personal, s.myInvites, s.apiKeys] = await Promise.all([
...
function renderApiKeys(body) {
  ...
  body.appendChild(
    node(
      "div",
      "org-hint",
      "Other local apps can call AnyLM's OpenAI-compatible endpoint (POST /v1/chat/completions on the AnyLM backend) with one of these keys. Your policies, limits, and logging apply to that traffic too."
    )
  );
  const table = node("div", "org-table");
